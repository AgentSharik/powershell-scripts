...
# =========================================================================
# ОЧИСТКА ФОНОВЫХ ПРОЦЕССОВ (Освобождение дескрипторов для выхода из скрипта)
# =========================================================================
Write-Host ">>> Оптимизация фоновых процессов после установки..."

$BloatProcesses = @("Steam", "steamwebhelper", "ShareX")
foreach ($ProcName in $BloatProcesses) {
    $TargetProc = Get-Process -Name $ProcName -ErrorAction SilentlyContinue
    if ($TargetProc) {
        Write-Host "Закрытие зависшего фонового процесса: $ProcName..."
        Stop-Process -Name $ProcName -Force -ErrorAction SilentlyContinue
    }
}

# Небольшая пауза, чтобы Windows успела освободить файлы логов
