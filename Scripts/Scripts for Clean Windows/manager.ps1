...
# --- СПИСОК ОПЕРАЦИЙ (ДОБАВЛЕН initial-setup.ps1) ---
$ScriptsToRun = @(
    @{ Name = "initial-setup.ps1";          Title = "Первоначальная настройка ОС";     Status = "Ожидание" },
...
$ProgressContainer.Add_Paint({
    ...
    if ($Global:CurrentFillWidth -gt 0) {
        ...
        if ($GBrushRect.Width -gt 0 -and $GBrushRect.Height -gt 0) {
            ...
            $ColorBlend.Colors = @([System.Drawing.Color]::FromArgb(0, 137, 180, 250), [System.Drawing.Color]::FromArgb(210, 255, 255, 255), [System.Drawing.Color]::FromArgb(0, 137, 180, 250))
...
$AnimTimer.Add_Tick({
    if ($Global:CurrentFillWidth -gt 0) {
        ...
        if ($Global:GlarePositionX -gt $Global:CurrentFillWidth) { $Global:GlarePositionX = -$Global:GlareWidth }
    } else { $Global:GlarePositionX = -$Global:GlareWidth }
...
$LaunchTimer.Add_Tick({
    ...
    for ($i = 0; $i -lt $ScriptsToRun.Count; $i++) {
        ...
        # ЛОГИРОВАНИЕ (ДОБАВЛЕН initial-setup)
        switch ($Script.Name) {
            "initial-setup.ps1"          { Add-LogLine "TASK" "Первоначальная настройка ОС" }
    ...
    if (Test-Path $ResetPath) {
        ...
        while (-not $ProcReset.HasExited) { [System.Windows.Forms.Application]::DoEvents(); Start-Sleep -Milliseconds 40 }
    }
