...
$LogFile = Join-Path $LogDir "System_Components_Install.log"

# Функция для моментальной записи в лог (для GUI менеджера)
function Write-Log {
...
function Install-Executable {
    ...
    try {
        ...
        if ($p.ExitCode -eq 3010) {
            Write-Log ">>> Компонент установлен успешно, но для завершения требуется перезагрузка (Код 3010)."
        } elseif ($p.ExitCode -eq 1638) {
            Write-Log ">>> Компонент (или его более новая версия) уже есть в системе. Установка не требуется (Код 1638)."
...
try {
    ...
    # ----------------------------------------------------
    Write-Log ">>> Шаг 1: Скачивание и тихая установка Visual C++ (x86 и x64)..."
    ...
    try {
        Write-Log "Скачивание Visual C++ x86..."
        Download-SetupFile -Uri $Vcx86Uri -OutFile $Vcx86File
        Write-Log "Установка Visual C++ x86..."
        Install-Executable -Path $Vcx86File -Arguments "/q /norestart"
        
        Write-Log "Скачивание Visual C++ x64..."
        Download-SetupFile -Uri $Vcx64Uri -OutFile $Vcx64File
        Write-Log "Установка Visual C++ x64..."
        Install-Executable -Path $Vcx64File -Arguments "/q /norestart"
        
        Write-Log ">>> Обработка пакетов Microsoft Visual C++ завершена."
    } catch {
        Write-Log "WARNING: Не удалось завершить установку Microsoft Visual C++: $($_.Exception.Message)"
    } finally {
    ...
    # ----------------------------------------------------
    Write-Log ">>> Шаг 2: Скачивание и тихая установка DirectX End-User Runtimes..."
    ...
    try {
        ...
        Write-Log ">>> Все компоненты DirectX успешно установлены."
    } catch {
        Write-Log "WARNING: Не удалось установить компоненты DirectX: $($_.Exception.Message)"
    } finally {
    ...
    # ----------------------------------------------------
    Write-Log ">>> Шаг 3: Включение .NET Framework 3.5..."
    try {
        Enable-WindowsOptionalFeature -Online -FeatureName "NetFx3" -NoRestart | Out-Null
        Write-Log ">>> .NET Framework 3.5 успешно активирован."
    } catch {
        Write-Log "WARNING: Не удалось активировать .NET Framework 3.5: $($_.Exception.Message)"
    }

    # ----------------------------------------------------
    # ЭТАП 4: УСТАНОВКА СОВРЕМЕННОГО .NET 8.0
    # ----------------------------------------------------
    Write-Log ">>> Шаг 4: Скачивание и установка .NET 8.0 Desktop Runtime (x64)..."
    try {
        ...
        Write-Log ">>> .NET 8.0 Desktop Runtime успешно установлен."
    } catch {
        Write-Log "WARNING: Не удалось установить .NET 8.0: $($_.Exception.Message)"
    } finally {
    ...
    # ----------------------------------------------------
    Write-Log ">>> Шаг 5: Скачивание и установка OpenAL (3D Audio)..."
    try {
        ...
        if (Test-Path $OalExe) {
            ...
            Write-Log ">>> Компоненты OpenAL успешно добавлены в систему."
        } else {
            throw "oalinst.exe не найден в корне архива."
        }
    } catch {
        Write-Log "WARNING: Не удалось установить OpenAL: $($_.Exception.Message)"
    } finally {
    ...
    Write-Log "ERROR: Произошел непредвиденный критический сбой ядра скрипта: $($_.Exception.Message)"
}
