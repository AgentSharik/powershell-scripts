# Проверка наличия прав администратора и перезапуск скрипта от имени админа
$isAdmin = ([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)

if (-not $isAdmin) {
    Write-Host "Скрипт запущен без прав администратора. Перезапуск..." -ForegroundColor Yellow
    # Перезапускаем этот же скрипт (-File $PSCommandPath) с правами админа (-Verb RunAs)
    Start-Process powershell.exe -ArgumentList "-NoProfile -ExecutionPolicy Bypass -File `"$PSCommandPath`"" -Verb RunAs
    exit # Завершаем текущий процесс без прав
}

Write-Host "Скрипт работает с правами администратора." -ForegroundColor Green
